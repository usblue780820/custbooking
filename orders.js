...
function renderTable() {
    ...
    const pageOrders = filtered.slice(ordersPage * ORDERS_PER_PAGE, (ordersPage + 1) * ORDERS_PER_PAGE);

    if(pagination) {
        if(totalPages > 1) {
            pagination.classList.remove('hidden');
            pageInfo.textContent = `第 ${ordersPage + 1} / ${totalPages} 頁（共 ${filtered.length} 筆）`;
            prevBtn.disabled = ordersPage === 0;
            nextBtn.disabled = ordersPage >= totalPages - 1;
        } else {
...
searchButton.addEventListener('click', () => { ordersPage = 0; renderTable(); });
searchInput.addEventListener('input', () => { ordersPage = 0; renderTable(); });
refreshButton.addEventListener('click', fetchOrders);

// 分頁按鈕
document.addEventListener('click', (e) => {
    if(e.target.id === 'prevPageBtn') { ordersPage--; renderTable(); }
    if(e.target.id === 'nextPageBtn') { ordersPage++; renderTable(); }
});
