...
async function fetchIssues() {
    ...
    try {
        ...
        if (tbody) tbody.innerHTML = '<tr><td colspan="9" class="py-6 text-center text-red-500">資料讀取失敗，請稍後再試</td></tr>';
...
function getIssueStatus(issue) {
    ...
    if (!notify) return { label: '未告知廠商', cls: 'bg-gray-100 text-gray-600' };
    if (!plan)   return { label: '已告知廠商', cls: 'bg-blue-100 text-blue-700' };
    const today = new Date(); today.setHours(0, 0, 0, 0);
    const planDate = new Date(plan.replace(/\//g, '-'));
    if (isNaN(planDate.getTime()) || planDate >= today) return { label: '處理中', cls: 'bg-yellow-100 text-yellow-700' };
    return { label: '逾期未回', cls: 'bg-red-100 text-red-700 font-bold' };
}
...
function renderIssuesList(issues) {
    ...
    tbody.innerHTML = issues.map(issue => {
        ...
        return `<tr class="border-b border-gray-100 hover:bg-amber-50 transition-colors cursor-pointer ${hasPhoto}" onclick="openEditIssue(${issue.__row})">
